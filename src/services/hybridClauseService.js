var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
import { apiCall } from './api';
import { urlBasedApiCall } from './urlBasedApi';
import { dlog } from '../utils/debugLog';
var ENABLE_URL_BASED_ROUTING = import.meta.env.VITE_ENABLE_URL_BASED_ROUTING === 'true';
// Helper function to get context from localStorage (simplified approach)
var getContextFromStorage = function () {
    if (typeof window === 'undefined')
        return null;
    var orgId = localStorage.getItem('orgId');
    var projectId = localStorage.getItem('projectId');
    // For now, return null to use header-based fallback
    // This would need to be enhanced to get slugs from context
    return null;
};
export var hybridClauseService = {
    getAllClauses: function () { return __awaiter(void 0, void 0, void 0, function () {
        var context, response, _a, error_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 5, , 6]);
                    context = getContextFromStorage();
                    if (!(ENABLE_URL_BASED_ROUTING && context)) return [3 /*break*/, 2];
                    return [4 /*yield*/, urlBasedApiCall('/clauses', {}, context)];
                case 1:
                    _a = _b.sent();
                    return [3 /*break*/, 4];
                case 2: return [4 /*yield*/, apiCall('/api/clauses')];
                case 3:
                    _a = _b.sent();
                    _b.label = 4;
                case 4:
                    response = _a;
                    dlog('[API] getAllClauses', {
                        len: Array.isArray(response.data) ? response.data.length : 'n/a',
                        missing: Array.isArray(response.data) ? response.data.filter(function (c) { return !c.family; }).length : 'n/a',
                        urlBased: ENABLE_URL_BASED_ROUTING && context
                    });
                    return [2 /*return*/, response];
                case 5:
                    error_1 = _b.sent();
                    console.error('Error fetching clauses:', error_1);
                    return [2 /*return*/, {
                            data: [],
                            error: error_1 instanceof Error ? error_1.message : 'Failed to fetch clauses'
                        }];
                case 6: return [2 /*return*/];
            }
        });
    }); },
    getGraphData: function () { return __awaiter(void 0, void 0, void 0, function () {
        var context, response, _a, error_2;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 5, , 6]);
                    context = getContextFromStorage();
                    if (!(ENABLE_URL_BASED_ROUTING && context)) return [3 /*break*/, 2];
                    return [4 /*yield*/, urlBasedApiCall('/clauses/graph', {}, context)];
                case 1:
                    _a = _b.sent();
                    return [3 /*break*/, 4];
                case 2: return [4 /*yield*/, apiCall('/api/clauses/graph')];
                case 3:
                    _a = _b.sent();
                    _b.label = 4;
                case 4:
                    response = _a;
                    return [2 /*return*/, response];
                case 5:
                    error_2 = _b.sent();
                    console.error('Error fetching graph data:', error_2);
                    return [2 /*return*/, {
                            data: { nodes: [], links: [] },
                            error: error_2 instanceof Error ? error_2.message : 'Failed to fetch graph data'
                        }];
                case 6: return [2 /*return*/];
            }
        });
    }); },
    getClausesByFamily: function (family) { return __awaiter(void 0, void 0, void 0, function () {
        var context, response, _a, error_3;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 5, , 6]);
                    context = getContextFromStorage();
                    if (!(ENABLE_URL_BASED_ROUTING && context)) return [3 /*break*/, 2];
                    return [4 /*yield*/, urlBasedApiCall("/clauses/family/".concat(family.id), {}, context)];
                case 1:
                    _a = _b.sent();
                    return [3 /*break*/, 4];
                case 2: return [4 /*yield*/, apiCall("/api/clauses/family/".concat(family.id))];
                case 3:
                    _a = _b.sent();
                    _b.label = 4;
                case 4:
                    response = _a;
                    dlog('[API] getClausesByFamily', {
                        familyId: family.id,
                        len: Array.isArray(response.data) ? response.data.length : 'n/a',
                        missing: Array.isArray(response.data) ? response.data.filter(function (c) { return !c.family; }).length : 'n/a',
                        urlBased: ENABLE_URL_BASED_ROUTING && context
                    });
                    return [2 /*return*/, response];
                case 5:
                    error_3 = _b.sent();
                    console.error('Error fetching clauses by family:', error_3);
                    return [2 /*return*/, {
                            data: [],
                            error: error_3 instanceof Error ? error_3.message : 'Failed to fetch clauses by family'
                        }];
                case 6: return [2 /*return*/];
            }
        });
    }); },
    getClauseFamilies: function () { return __awaiter(void 0, void 0, void 0, function () {
        var context, response, _a, error_4;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 5, , 6]);
                    context = getContextFromStorage();
                    if (!(ENABLE_URL_BASED_ROUTING && context)) return [3 /*break*/, 2];
                    return [4 /*yield*/, urlBasedApiCall('/clauses/families', {}, context)];
                case 1:
                    _a = _b.sent();
                    return [3 /*break*/, 4];
                case 2: return [4 /*yield*/, apiCall('/api/clauses/families')];
                case 3:
                    _a = _b.sent();
                    _b.label = 4;
                case 4:
                    response = _a;
                    return [2 /*return*/, response];
                case 5:
                    error_4 = _b.sent();
                    console.error('Error fetching families:', error_4);
                    return [2 /*return*/, {
                            data: [],
                            error: error_4 instanceof Error ? error_4.message : 'Failed to fetch families'
                        }];
                case 6: return [2 /*return*/];
            }
        });
    }); },
    getClauseById: function (id) { return __awaiter(void 0, void 0, void 0, function () {
        var context, response, _a, error_5;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 5, , 6]);
                    context = getContextFromStorage();
                    if (!(ENABLE_URL_BASED_ROUTING && context)) return [3 /*break*/, 2];
                    return [4 /*yield*/, urlBasedApiCall("/clauses/".concat(id), {}, context)];
                case 1:
                    _a = _b.sent();
                    return [3 /*break*/, 4];
                case 2: return [4 /*yield*/, apiCall("/api/clauses/".concat(id))];
                case 3:
                    _a = _b.sent();
                    _b.label = 4;
                case 4:
                    response = _a;
                    return [2 /*return*/, response];
                case 5:
                    error_5 = _b.sent();
                    console.error('Error fetching clause:', error_5);
                    return [2 /*return*/, {
                            data: null,
                            error: error_5 instanceof Error ? error_5.message : 'Failed to fetch clause'
                        }];
                case 6: return [2 /*return*/];
            }
        });
    }); },
    searchClauses: function (query) { return __awaiter(void 0, void 0, void 0, function () {
        var context, response, _a, error_6;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 5, , 6]);
                    context = getContextFromStorage();
                    if (!(ENABLE_URL_BASED_ROUTING && context)) return [3 /*break*/, 2];
                    return [4 /*yield*/, urlBasedApiCall("/clauses/search?q=".concat(encodeURIComponent(query)), {}, context)];
                case 1:
                    _a = _b.sent();
                    return [3 /*break*/, 4];
                case 2: return [4 /*yield*/, apiCall("/api/clauses/search?q=".concat(encodeURIComponent(query)))];
                case 3:
                    _a = _b.sent();
                    _b.label = 4;
                case 4:
                    response = _a;
                    return [2 /*return*/, response];
                case 5:
                    error_6 = _b.sent();
                    console.error('Error searching clauses:', error_6);
                    return [2 /*return*/, {
                            data: [],
                            error: error_6 instanceof Error ? error_6.message : 'Failed to search clauses'
                        }];
                case 6: return [2 /*return*/];
            }
        });
    }); },
    bookmarkClause: function (clauseId) { return __awaiter(void 0, void 0, void 0, function () {
        var context, response, _a, payload, error_7;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 5, , 6]);
                    context = getContextFromStorage();
                    if (!(ENABLE_URL_BASED_ROUTING && context)) return [3 /*break*/, 2];
                    return [4 /*yield*/, urlBasedApiCall("/clauses/".concat(clauseId, "/bookmark"), { method: 'POST' }, context)];
                case 1:
                    _a = _b.sent();
                    return [3 /*break*/, 4];
                case 2: return [4 /*yield*/, apiCall("/api/clauses/".concat(clauseId, "/bookmark"), { method: 'POST' })];
                case 3:
                    _a = _b.sent();
                    _b.label = 4;
                case 4:
                    response = _a;
                    if (response.error) {
                        return [2 /*return*/, response];
                    }
                    payload = response.data;
                    if (payload && typeof payload === 'object' && 'data' in payload && payload.data) {
                        return [2 /*return*/, {
                                data: payload.data,
                                error: null
                            }];
                    }
                    return [2 /*return*/, response];
                case 5:
                    error_7 = _b.sent();
                    console.error('Error bookmarking clause:', error_7);
                    return [2 /*return*/, {
                            data: null,
                            error: error_7 instanceof Error ? error_7.message : 'Failed to bookmark clause'
                        }];
                case 6: return [2 /*return*/];
            }
        });
    }); }
};
