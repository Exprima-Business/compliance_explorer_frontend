...
/**
 * The guided layer that sits above Posture/Gaps/Moves (locked "guided-not-menu"
 * design): a stateful journey spine + a single explained next-step hero whose
 * content recomputes per stage. Mockup: docs/mockups/cascade-dashboard/
 * full-page-guided.html.
 */
...
interface Hero { title: string; why: string; whatNext: string; ctaLabel: string; onCta: () => void; }
...
function buildHero(stage: number, p: JourneyGuideProps): Hero {
  if (stage === 1) return {
    ...
    whatNext: "activate a curated framework bundle or answer a few questions — we surface your obligations automatically.",
    ctaLabel: 'Set up scope', onCta: p.onSetupScope,
  };
  if (stage === 2) return {
    title: 'Confirm your obligations',
    why: 'your frameworks are set; these are the obligations they trigger.',
    whatNext: 'review the surfaced requirements below and start closing them.',
    ctaLabel: 'View requirements', onCta: p.onViewAllMoves,
  };
  if (stage === 3 && p.topMove) {
    const m = p.topMove;
    const obl = `${m.obligationsCleared} ${m.obligationsCleared === 1 ? 'obligation' : 'obligations'}`;
    const auth = `${m.authoritiesCount} ${m.authoritiesCount === 1 ? 'authority' : 'authorities'}`;
    return {
      title: m.mechanismLabel,
      why: `it's your highest-leverage move — one action clears ${obl} across ${auth}.`,
      whatNext: 'open the requirements it resolves, mark them satisfied, and your posture climbs here.',
      ctaLabel: 'Start this move', onCta: () => p.onStartMove(m),
    };
  }
  if (stage === 4) return {
    title: "You're audit-ready — keep it that way",
    why: `${p.posture}% of your obligations are covered.`,
    whatNext: 'export your SSP / POA&M and set renewals so you stay ready.',
    ctaLabel: 'Generate report', onCta: p.onGenerateReport,
  };
  // Close-gaps with no ranked move yet.
  return {
    title: 'Work your open requirements',
    why: `${p.gapsOpen} ${p.gapsOpen === 1 ? 'requirement remains' : 'requirements remain'} open.`,
    whatNext: 'open each requirement to mark satisfaction and attach evidence.',
    ctaLabel: 'View requirements', onCta: p.onViewAllMoves,
  };
}
...
export default function JourneyGuide(props: JourneyGuideProps) {
  ...
  return (
    <Box sx={{ mb: 2 }}>
      ...
      {/* Next-step hero */}
      <Box sx={{ bgcolor: '#F5F4FC', border: '1px solid #D9D5F0', borderRadius: 2, p: '18px 20px' }}>
        <Typography sx={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase', color: PURPLE_DK, mb: 0.75 }}>
          Your next step
        </Typography>
        <Typography sx={{ fontSize: 18, fontWeight: 600, mb: 1 }}>{hero.title}</Typography>
        <Typography sx={{ fontSize: 13, color: 'text.secondary', lineHeight: 1.6, mb: 0.5 }}>
          <BoltIcon sx={{ fontSize: 14, color: PURPLE_DK, verticalAlign: -2, mr: 0.5 }} />
          <Box component="span" sx={{ color: 'text.primary', fontWeight: 500 }}>Why this:</Box> {hero.why}
        </Typography>
        <Typography sx={{ fontSize: 13, color: 'text.secondary', lineHeight: 1.6, mb: 2 }}>
          <ArrowForwardIcon sx={{ fontSize: 14, color: PURPLE_DK, verticalAlign: -2, mr: 0.5 }} />
          <Box component="span" sx={{ color: 'text.primary', fontWeight: 500 }}>What happens next:</Box> {hero.whatNext}
        </Typography>
        <Stack direction="row" alignItems="center" spacing={1.5} sx={{ flexWrap: 'wrap', gap: 1 }}>
          <Button variant="contained" sx={{ bgcolor: PURPLE, textTransform: 'none', fontWeight: 500 }}
            endIcon={<ArrowForwardIcon />} onClick={hero.onCta}>
            {hero.ctaLabel}
          </Button>
          {stage === 3 && props.topMove && props.moveCount > 1 && (
            <Typography sx={{ fontSize: 13, color: PURPLE_DK, cursor: 'pointer' }} onClick={props.onViewAllMoves}>
              See all {props.moveCount} moves
            </Typography>
          )}
          <Box sx={{ flex: 1 }} />
          {props.total > 0 && (
            <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>
              {props.covered} of {props.total} requirements covered
            </Typography>
          )}
        </Stack>
      </Box>
